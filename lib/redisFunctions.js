...
const hmset = function (client, jobId, jobDetails) {
  return new Promise((resolve, reject) => {
    client.hmset(jobId, jobDetails, (err) => {
      if (err) return reject('Unable to create job', jobId);
      resolve(jobId);
    });
  });
};
...
const sadd = function (client, key, element) {
  return new Promise((resolve, reject) => {
    client.sadd(key, element, (err, result) => {
      if (err) return reject(err);
      resolve(result);
    });
  });
...
module.exports = { increment, lpush, hmset, hgetall, brpop, sadd };
